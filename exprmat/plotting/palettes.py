'''
Collection of palettes for color mapping.

A palette is a simple plain Python list of (hex) RGB color strings. 
For example the ``Blues8`` palette is defined as:
...
Every pre-built palette is available as a module attributes, There are also 
functions such as :func:`palettes.magma` and `palettes.viridis` that can generate 
lists of colors of arbitrary size from special larger palettes.

The Brewer palettes are also collected and grouped by name in a ``brewer`` dictionary, 
e.g.: ``brewer['Spectral'][6]``. Similarly there are attributes ``d3``, ``mpl``, and
``tol`` that have dictionaries corresponding to the those groups of palettes.
...
different usage. ``PaleTextBackground`` should be used for the background of black text. 
``DarkText`` is meant for text itself on a white background. The idea is to use one 
dark color for support, not all combined and not for just one word.
'''
...
class color:
    ...
    @classmethod
    @abstractmethod
    def from_hsl(cls, value: hsl_color):
        raise NotImplementedError
    ...
    @abstractmethod
    def to_hsl(self) -> hsl_color:
        raise NotImplementedError

    @abstractmethod
    def to_rgb(self) -> rgb_color:
        raise NotImplementedError
...
class rgb_color(color):
    ...
    def copy(self) -> rgb_color:
        return rgb_color(self.r, self.g, self.b, self.a)

    @classmethod
    def from_hsl(cls, value: hsl_color) -> rgb_color:
        return value.to_rgb()

    @classmethod
    def from_hex_string(cls, hex_string: str) -> rgb_color:
    ...
    @classmethod
    def from_tuple(cls, value: rgb_tuple) -> rgb_color:
        if len(value) == 3:
            r, g, b = value
    ...
    @classmethod
    def from_rgb(cls, value: rgb_color) -> rgb_color:
        return value.copy()

    def to_css(self) -> str:
        if self.a == 1.0: return f"rgb({self.r}, {self.g}, {self.b})"
        else: return f"rgba({self.r}, {self.g}, {self.b}, {self.a})"

    def to_hex(self) -> str:
        if self.a < 1.0: return f"#{self.r:02x}{self.g:02x}{self.b:02x}{round(self.a*255):02x}"
        else: return f"#{self.r:02x}{self.g:02x}{self.b:02x}"

    def to_hsl(self) -> hsl_color:
        h, l, s = colorsys.rgb_to_hls(float(self.r)/255, float(self.g)/255, float(self.b)/255)
        return hsl_color(round(h*360), s, l, self.a)
...
class hsl_color(color):
    ...
    def copy(self) -> hsl_color:
        return hsl_color(self.h, self.s, self.l, self.a)

    def darken(self, amount: float) -> hsl_color:
        return self.lighten(-amount)

    @classmethod
    def from_hsl(cls, value: hsl_color) -> hsl_color:
        return value.copy()

    @classmethod
    def from_rgb(cls, value: rgb_color) -> hsl_color:
        return value.to_hsl()

    def to_css(self) -> str:
        if self.a == 1.0: return f"hsl({self.h}, {self.s*100}%, {self.l*100}%)"
        else: return f"hsla({self.h}, {self.s*100}%, {self.l*100}%, {self.a})"
    ...
    def to_rgb(self) -> rgb_color:
        ...
        return rgb_color(round(r * 255), round(g * 255), round(b * 255), self.a)

    def lighten(self, amount: float) -> hsl_color:
        hsl = self.copy()
        hsl.l = self.clamp(hsl.l + amount, 1)
        return self.from_hsl(hsl)


class named_color(rgb_color):

    __all__: ClassVar[list[str]] = []
    colors: ClassVar[list[named_color]] = []
    ...
    @classmethod
    def find(cls, name: str) -> named_color | None:
        try: index = cls.__all__.index(name)
        except ValueError: return None
        return cls.colors[index]

    @classmethod
    def from_string(cls, color: str) -> rgb_color:
        try:
            # Is it a hex string?
            return rgb_color.from_hex_string(color)
        except ValueError:
            # Is it a named color?
            rgb = cls.find(color)
            if rgb is None: error(f'invalid color string: {color}')
            return rgb
...
aliceblue            = named_color("aliceblue",             240,  248,  255)
antiquewhite         = named_color("antiquewhite",          250,  235,  215)
aqua                 = named_color("aqua",                  0,    255,  255)
aquamarine           = named_color("aquamarine",            127,  255,  212)
azure                = named_color("azure",                 240,  255,  255)
beige                = named_color("beige",                 245,  245,  220)
bisque               = named_color("bisque",                255,  228,  196)
black                = named_color("black",                 0,    0,    0  )
blanchedalmond       = named_color("blanchedalmond",        255,  235,  205)
blue                 = named_color("blue",                  0,    0,    255)
blueviolet           = named_color("blueviolet",            138,  43,   226)
brown                = named_color("brown",                 165,  42,   42 )
burlywood            = named_color("burlywood",             222,  184,  135)
cadetblue            = named_color("cadetblue",             95,   158,  160)
chartreuse           = named_color("chartreuse",            127,  255,  0  )
chocolate            = named_color("chocolate",             210,  105,  30 )
coral                = named_color("coral",                 255,  127,  80 )
cornflowerblue       = named_color("cornflowerblue",        100,  149,  237)
cornsilk             = named_color("cornsilk",              255,  248,  220)
crimson              = named_color("crimson",               220,  20,   60 )
cyan                 = named_color("cyan",                  0,    255,  255)
darkblue             = named_color("darkblue",              0,    0,    139)
darkcyan             = named_color("darkcyan",              0,    139,  139)
darkgoldenrod        = named_color("darkgoldenrod",         184,  134,  11 )
darkgray             = named_color("darkgray",              169,  169,  169)
darkgreen            = named_color("darkgreen",             0,    100,  0  )
darkgrey             = named_color("darkgrey",              169,  169,  169)
darkkhaki            = named_color("darkkhaki",             189,  183,  107)
darkmagenta          = named_color("darkmagenta",           139,  0,    139)
darkolivegreen       = named_color("darkolivegreen",        85,   107,  47 )
darkorange           = named_color("darkorange",            255,  140,  0  )
darkorchid           = named_color("darkorchid",            153,  50,   204)
darkred              = named_color("darkred",               139,  0,    0  )
darksalmon           = named_color("darksalmon",            233,  150,  122)
darkseagreen         = named_color("darkseagreen",          143,  188,  143)
darkslateblue        = named_color("darkslateblue",         72,   61,   139)
darkslategray        = named_color("darkslategray",         47,   79,   79 )
darkslategrey        = named_color("darkslategrey",         47,   79,   79 )
darkturquoise        = named_color("darkturquoise",         0,    206,  209)
darkviolet           = named_color("darkviolet",            148,  0,    211)
deeppink             = named_color("deeppink",              255,  20,   147)
deepskyblue          = named_color("deepskyblue",           0,    191,  255)
dimgray              = named_color("dimgray",               105,  105,  105)
dimgrey              = named_color("dimgrey",               105,  105,  105)
dodgerblue           = named_color("dodgerblue",            30,   144,  255)
firebrick            = named_color("firebrick",             178,  34,   34 )
floralwhite          = named_color("floralwhite",           255,  250,  240)
forestgreen          = named_color("forestgreen",           34,   139,  34 )
fuchsia              = named_color("fuchsia",               255,  0,    255)
gainsboro            = named_color("gainsboro",             220,  220,  220)
ghostwhite           = named_color("ghostwhite",            248,  248,  255)
gold                 = named_color("gold",                  255,  215,  0  )
goldenrod            = named_color("goldenrod",             218,  165,  32 )
gray                 = named_color("gray",                  128,  128,  128)
green                = named_color("green",                 0,    128,  0  )
greenyellow          = named_color("greenyellow",           173,  255,  47 )
grey                 = named_color("grey",                  128,  128,  128)
honeydew             = named_color("honeydew",              240,  255,  240)
hotpink              = named_color("hotpink",               255,  105,  180)
indianred            = named_color("indianred",             205,  92,   92 )
indigo               = named_color("indigo",                75,   0,    130)
ivory                = named_color("ivory",                 255,  255,  240)
khaki                = named_color("khaki",                 240,  230,  140)
lavender             = named_color("lavender",              230,  230,  250)
lavenderblush        = named_color("lavenderblush",         255,  240,  245)
lawngreen            = named_color("lawngreen",             124,  252,  0  )
lemonchiffon         = named_color("lemonchiffon",          255,  250,  205)
lightblue            = named_color("lightblue",             173,  216,  230)
lightcoral           = named_color("lightcoral",            240,  128,  128)
lightcyan            = named_color("lightcyan",             224,  255,  255)
lightgoldenrodyellow = named_color("lightgoldenrodyellow",  250,  250,  210)
lightgray            = named_color("lightgray",             211,  211,  211)
lightgreen           = named_color("lightgreen",            144,  238,  144)
lightgrey            = named_color("lightgrey",             211,  211,  211)
lightpink            = named_color("lightpink",             255,  182,  193)
lightsalmon          = named_color("lightsalmon",           255,  160,  122)
lightseagreen        = named_color("lightseagreen",         32,   178,  170)
lightskyblue         = named_color("lightskyblue",          135,  206,  250)
lightslategray       = named_color("lightslategray",        119,  136,  153)
lightslategrey       = named_color("lightslategrey",        119,  136,  153)
lightsteelblue       = named_color("lightsteelblue",        176,  196,  222)
lightyellow          = named_color("lightyellow",           255,  255,  224)
lime                 = named_color("lime",                  0,    255,  0  )
limegreen            = named_color("limegreen",             50,   205,  50 )
linen                = named_color("linen",                 250,  240,  230)
magenta              = named_color("magenta",               255,  0,    255)
maroon               = named_color("maroon",                128,  0,    0  )
mediumaquamarine     = named_color("mediumaquamarine",      102,  205,  170)
mediumblue           = named_color("mediumblue",            0,    0,    205)
mediumorchid         = named_color("mediumorchid",          186,  85,   211)
mediumpurple         = named_color("mediumpurple",          147,  112,  219)
mediumseagreen       = named_color("mediumseagreen",        60,   179,  113)
mediumslateblue      = named_color("mediumslateblue",       123,  104,  238)
mediumspringgreen    = named_color("mediumspringgreen",     0,    250,  154)
mediumturquoise      = named_color("mediumturquoise",       72,   209,  204)
mediumvioletred      = named_color("mediumvioletred",       199,  21,   133)
midnightblue         = named_color("midnightblue",          25,   25,   112)
mintcream            = named_color("mintcream",             245,  255,  250)
mistyrose            = named_color("mistyrose",             255,  228,  225)
moccasin             = named_color("moccasin",              255,  228,  181)
navajowhite          = named_color("navajowhite",           255,  222,  173)
navy                 = named_color("navy",                  0,    0,    128)
oldlace              = named_color("oldlace",               253,  245,  230)
olive                = named_color("olive",                 128,  128,  0  )
olivedrab            = named_color("olivedrab",             107,  142,  35 )
orange               = named_color("orange",                255,  165,  0  )
orangered            = named_color("orangered",             255,  69,   0  )
orchid               = named_color("orchid",                218,  112,  214)
palegoldenrod        = named_color("palegoldenrod",         238,  232,  170)
palegreen            = named_color("palegreen",             152,  251,  152)
paleturquoise        = named_color("paleturquoise",         175,  238,  238)
palevioletred        = named_color("palevioletred",         219,  112,  147)
papayawhip           = named_color("papayawhip",            255,  239,  213)
peachpuff            = named_color("peachpuff",             255,  218,  185)
peru                 = named_color("peru",                  205,  133,  63 )
pink                 = named_color("pink",                  255,  192,  203)
plum                 = named_color("plum",                  221,  160,  221)
powderblue           = named_color("powderblue",            176,  224,  230)
purple               = named_color("purple",                128,  0,    128)
rebeccapurple        = named_color("rebeccapurple",         102,  51,   153)
red                  = named_color("red",                   255,  0,    0  )
rosybrown            = named_color("rosybrown",             188,  143,  143)
royalblue            = named_color("royalblue",             65,   105,  225)
saddlebrown          = named_color("saddlebrown",           139,  69,   19 )
salmon               = named_color("salmon",                250,  128,  114)
sandybrown           = named_color("sandybrown",            244,  164,  96 )
seagreen             = named_color("seagreen",              46,   139,  87 )
seashell             = named_color("seashell",              255,  245,  238)
sienna               = named_color("sienna",                160,  82,   45 )
silver               = named_color("silver",                192,  192,  192)
skyblue              = named_color("skyblue",               135,  206,  235)
slateblue            = named_color("slateblue",             106,  90,   205)
slategray            = named_color("slategray",             112,  128,  144)
slategrey            = named_color("slategrey",             112,  128,  144)
snow                 = named_color("snow",                  255,  250,  250)
springgreen          = named_color("springgreen",           0,    255,  127)
steelblue            = named_color("steelblue",             70,   130,  180)
tan                  = named_color("tan",                   210,  180,  140)
teal                 = named_color("teal",                  0,    128,  128)
thistle              = named_color("thistle",               216,  191,  216)
tomato               = named_color("tomato",                255,  99,   71 )
turquoise            = named_color("turquoise",             64,   224,  208)
violet               = named_color("violet",                238,  130,  238)
wheat                = named_color("wheat",                 245,  222,  179)
white                = named_color("white",                 255,  255,  255)
whitesmoke           = named_color("whitesmoke",            245,  245,  245)
yellow               = named_color("yellow",                255,  255,  0  )
yellowgreen          = named_color("yellowgreen",           154,  205,  50 )


ColorLike: TypeAlias = str | color | rgb_tuple
palette: TypeAlias = tuple[str, ...]
palette_collection: TypeAlias = dict[int, palette]
palette_map: TypeAlias = dict[str, palette_collection]
...
def linear_palette(palette: palette, n: int) -> palette:
    ...
    Given an input ``palette``, take ``n`` colors from it by dividing its length into 
    ``n`` (approximately) evenly spaced indices. Raises ValueError if n > len(palette)
    '''
    if n > len(palette):
...
def diverging_palette(palette1: palette, palette2: palette, n: int, midpoint: float = 0.5) -> palette:
    ...
    Raises ValueError if n is greater than the possible combined length the input palettes
    '''
...
def varying_alpha_palette(
    color: str, n: int | None = None, 
    start_alpha: int = 0, 
    end_alpha: int = 255
) -> palette:
    '''
    ...
    Returns
    -----------
    A sequence of hex RGBA color strings. Raises ValueError if ``color`` 
    is not recognizable as a string name or hex RGB(A) string, or if ``start_alpha`` 
    or ``end_alpha`` are outside the range 0 to 255 inclusive.
    '''
    ...
    rgba = named_color.from_string(color).copy()
    ...
    def set_alpha(rgba: rgb_color, i: int) -> rgb_color:
...
def interp_palette(palette: palette, n: int) -> palette:
    ...
    return tuple(rgb_color(*args).to_hex() for args in zip(r, g, b, a))
