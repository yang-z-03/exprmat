...
import exprmat.deseq
from exprmat.deseq.gridsearch import grid_fit_alpha
from exprmat.deseq.gridsearch import grid_fit_beta
from exprmat.deseq.gridsearch import grid_fit_shrink_beta
...
def dnb_nll(counts: np.ndarray, mu: np.ndarray, alpha: float) -> float:
    r"""
    Gradient of the negative log-likelihood of a negative binomial, unvectorized.
...
def nbinom_glm(
    ...
    scale_cnst = nbinom_negative_likelihood(
        np.zeros(num_vars),
        design_matrix,
        counts,
        size,
...
def nbinom_negative_likelihood(
    beta: np.ndarray,
    design_matrix: np.ndarray,
    counts: np.ndarray,
    size: np.ndarray,
