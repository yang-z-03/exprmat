...
def grid_fit_shrink_beta(
    ...
    """
    Find best LFC parameter. Performs 2D grid search to maximize MAP negative 
    binomial GLM log-likelihood w.r.t. LFCs, with apeGLM prior.
    ...
    def loss(beta: np.ndarray) -> float:
        # closure to minimize
        return (
            exprmat.deseq.utils.nbinom_negative_likelihood(
                beta,
                design_matrix,
                counts,
                size,
