...
def py_cdf(x, mu, sigma, lamda):
    norm_x = (x - mu) / sigma
    return 0.5 * (
        (1 - lamda) * erf(0.707106781186547461715 * norm_x) 
        + lamda * erf(0.707106781186547461715 * x) + 1 
    )

def py_cdf_i(r, mu, sigma, pi, lb, ub, eps):
    return brentq(lambda x: cdf(x, mu, sigma, pi) - r, lb, ub)

def py_compute_pseudo_values(
    ranks, signal_mu, signal_sd, p, 
    EPS = DEFAULT_PV_COVERGE_EPS
):
    pseudo_values = []
    for x in ranks:
        new_x = float(x+1)/(len(ranks)+1)
        pseudo_values.append(cdf_i(
            new_x, signal_mu, signal_sd, p, 
            -10, 10, EPS 
        ))
...
try: 
    from exprmat.peaks.idr.invcdf import cdf, cdf_i, c_compute_pseudo_values
...
def calc_gaussian_mix_log_lhd_gradient(theta, z1, z2, fix_mu, fix_sigma):
    ...
    if fix_mu: mu_grad = 0
    else: mu_grad = (ez * ((std_z1 + std_z2) / (1 - rho * rho))).sum()

    if fix_sigma: sigma_grad = 0
